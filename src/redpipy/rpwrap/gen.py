...
def reset() -> None:
    ...
    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenReset", (), __status_code)
...
def out_enable(channel: constants.Channel) -> None:
    ...
    __status_code = rp.rp_GenOutEnable(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenOutEnable", (channel,), __status_code)
...
def out_enable_sync(enable: bool) -> None:
    ...
    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenOutEnableSync", (enable,), __status_code)
...
def out_disable(channel: constants.Channel) -> None:
    ...
    __status_code = rp.rp_GenOutDisable(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenOutDisable", (channel,), __status_code)
...
def out_is_enabled(channel: constants.Channel, value: bool) -> None:
    ...
    __status_code = rp.rp_GenOutIsEnabled(channel, value)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenOutIsEnabled", (channel, "<value>"), __status_code
        )
...
def amp(channel: constants.Channel, amplitude: float) -> None:
    ...
    __status_code = rp.rp_GenAmp(channel, amplitude)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenAmp", (channel, amplitude), __status_code)
...
def get_amp(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetAmp(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetAmp", (channel,), __status_code)
...
def offset(channel: constants.Channel, offset: float) -> None:
    ...
    __status_code = rp.rp_GenOffset(channel, offset)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenOffset", (channel, offset), __status_code)
...
def get_offset(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetOffset(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetOffset", (channel,), __status_code)
...
def freq(channel: constants.Channel, frequency: float) -> None:
    ...
    __status_code = rp.rp_GenFreq(channel, frequency)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenFreq", (channel, frequency), __status_code)
...
def freq_direct(channel: constants.Channel, frequency: float) -> None:
    ...
    __status_code = rp.rp_GenFreqDirect(channel, frequency)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenFreqDirect", (channel, frequency), __status_code
        )
...
def get_freq(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetFreq(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetFreq", (channel,), __status_code)
...
def sweep_start_freq(channel: constants.Channel, frequency: float) -> None:
    ...
    __status_code = rp.rp_GenSweepStartFreq(channel, frequency)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenSweepStartFreq", (channel, frequency), __status_code
        )
...
def get_sweep_start_freq(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetSweepStartFreq(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetSweepStartFreq", (channel,), __status_code)
...
def sweep_end_freq(channel: constants.Channel, frequency: float) -> None:
    ...
    __status_code = rp.rp_GenSweepEndFreq(channel, frequency)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenSweepEndFreq", (channel, frequency), __status_code
        )
...
def get_sweep_end_freq(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetSweepEndFreq(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetSweepEndFreq", (channel,), __status_code)
...
def phase(channel: constants.Channel, phase: float) -> None:
    ...
    __status_code = rp.rp_GenPhase(channel, phase)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenPhase", (channel, phase), __status_code)
...
def get_phase(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetPhase(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetPhase", (channel,), __status_code)
...
def waveform(channel: constants.Channel, type: constants.Waveform) -> None:
    ...
    __status_code = rp.rp_GenWaveform(channel, type)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenWaveform", (channel, type), __status_code)
...
def get_waveform(channel: constants.Channel) -> constants.Waveform:
    ...
    __status_code, __value = rp.rp_GenGetWaveform(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetWaveform", (channel,), __status_code)
...
def sweep_mode(channel: constants.Channel, mode: constants.GenSweepMode) -> None:
    ...
    __status_code = rp.rp_GenSweepMode(channel, mode)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenSweepMode", (channel, mode), __status_code)
...
def get_sweep_mode(channel: constants.Channel) -> constants.GenSweepMode:
    ...
    __status_code, __value = rp.rp_GenGetSweepMode(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetSweepMode", (channel,), __status_code)
...
def sweep_dir(channel: constants.Channel, mode: constants.GenSweepDirection) -> None:
    ...
    __status_code = rp.rp_GenSweepDir(channel, mode)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenSweepDir", (channel, mode), __status_code)
...
def get_sweep_dir(channel: constants.Channel) -> constants.GenSweepDirection:
    ...
    __status_code, __value = rp.rp_GenGetSweepDir(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetSweepDir", (channel,), __status_code)
...
def arb_waveform(channel: constants.Channel, waveform: float, length: int) -> None:
    ...
    __status_code = rp.rp_GenArbWaveform(channel, waveform, length)
...
def get_arb_waveform(
    ...
    __status_code, __value = rp.rp_GenGetArbWaveform(channel, waveform, length)
...
def duty_cycle(channel: constants.Channel, ratio: float) -> None:
    ...
    __status_code = rp.rp_GenDutyCycle(channel, ratio)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenDutyCycle", (channel, ratio), __status_code)
...
def get_duty_cycle(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetDutyCycle(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetDutyCycle", (channel,), __status_code)
...
def rise_time(channel: constants.Channel, time: float) -> None:
    ...
    __status_code = rp.rp_GenRiseTime(channel, time)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenRiseTime", (channel, time), __status_code)


def get_rise_time(channel: constants.Channel) -> float:
    """ """

    __status_code, __value = rp.rp_GenGetRiseTime(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetRiseTime", (channel,), __status_code)
...
def fall_time(channel: constants.Channel, time: float) -> None:
    ...
    __status_code = rp.rp_GenFallTime(channel, time)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenFallTime", (channel, time), __status_code)


def get_fall_time(channel: constants.Channel) -> float:
    """ """

    __status_code, __value = rp.rp_GenGetFallTime(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetFallTime", (channel,), __status_code)
...
def mode(channel: constants.Channel, mode: constants.GenMode) -> None:
    ...
    __status_code = rp.rp_GenMode(channel, mode)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenMode", (channel, mode), __status_code)
...
def get_mode(channel: constants.Channel) -> constants.GenMode:
    ...
    __status_code, __value = rp.rp_GenGetMode(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetMode", (channel,), __status_code)
...
def burst_count(channel: constants.Channel, num: int) -> None:
    ...
    __status_code = rp.rp_GenBurstCount(channel, num)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenBurstCount", (channel, num), __status_code)
...
def get_burst_count(channel: constants.Channel) -> int:
    ...
    __status_code, __value = rp.rp_GenGetBurstCount(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetBurstCount", (channel,), __status_code)
...
def burst_last_value(channel: constants.Channel, amplitude: float) -> None:
    ...
    __status_code = rp.rp_GenBurstLastValue(channel, amplitude)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenBurstLastValue", (channel, amplitude), __status_code
        )
...
def get_burst_last_value(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetBurstLastValue(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetBurstLastValue", (channel,), __status_code)
...
def set_init_gen_value(channel: constants.Channel, amplitude: float) -> None:
    ...
    __status_code = rp.rp_GenSetInitGenValue(channel, amplitude)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenSetInitGenValue", (channel, amplitude), __status_code
        )
...
def get_init_gen_value(channel: constants.Channel) -> float:
    ...
    __status_code, __value = rp.rp_GenGetInitGenValue(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetInitGenValue", (channel,), __status_code)
...
def burst_repetitions(channel: constants.Channel, repetitions: int) -> None:
    ...
    __status_code = rp.rp_GenBurstRepetitions(channel, repetitions)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenBurstRepetitions", (channel, repetitions), __status_code
        )
...
def get_burst_repetitions(channel: constants.Channel) -> int:
    ...
    __status_code, __value = rp.rp_GenGetBurstRepetitions(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetBurstRepetitions", (channel,), __status_code)
...
def burst_period(channel: constants.Channel, period: int) -> None:
    ...
    __status_code = rp.rp_GenBurstPeriod(channel, period)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenBurstPeriod", (channel, period), __status_code)
...
def get_burst_period(channel: constants.Channel) -> int:
    ...
    __status_code, __value = rp.rp_GenGetBurstPeriod(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetBurstPeriod", (channel,), __status_code)
...
def trigger_source(channel: constants.Channel, src: constants.TriggerSource) -> None:
    ...
    __status_code = rp.rp_GenTriggerSource(channel, src)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenTriggerSource", (channel, src), __status_code)
...
def get_trigger_source(channel: constants.Channel) -> constants.TriggerSource:
    ...
    __status_code, __value = rp.rp_GenGetTriggerSource(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetTriggerSource", (channel,), __status_code)
...
def synchronise() -> None:
    ...
    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenSynchronise", (), __status_code)
...
def reset_trigger(channel: constants.Channel) -> None:
    ...
    __status_code = rp.rp_GenResetTrigger(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenResetTrigger", (channel,), __status_code)


def trigger_only(channel: constants.Channel) -> None:
    """ """

    __status_code = rp.rp_GenTriggerOnly(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenTriggerOnly", (channel,), __status_code)
...
def reset_channel_sm(channel: constants.Channel) -> None:
    ...
    __status_code = rp.rp_GenResetChannelSM(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenResetChannelSM", (channel,), __status_code)
...
def set_enable_temp_protection(channel: constants.Channel, enable: bool) -> None:
    ...
    __status_code = rp.rp_SetEnableTempProtection(channel, enable)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_SetEnableTempProtection", (channel, enable), __status_code
        )
...
def get_enable_temp_protection(channel: constants.Channel) -> bool:
    ...
    __status_code, __value = rp.rp_GetEnableTempProtection(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GetEnableTempProtection", (channel,), __status_code
        )
...
def set_latch_temp_alarm(channel: constants.Channel, status: bool) -> None:
    ...
    __status_code = rp.rp_SetLatchTempAlarm(channel, status)

    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_SetLatchTempAlarm", (channel, status), __status_code
        )
...
def get_latch_temp_alarm(channel: constants.Channel) -> bool:
    ...
    __status_code, __value = rp.rp_GetLatchTempAlarm(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GetLatchTempAlarm", (channel,), __status_code)
...
def get_runtime_temp_alarm(channel: constants.Channel) -> bool:
    ...
    __status_code, __value = rp.rp_GetRuntimeTempAlarm(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GetRuntimeTempAlarm", (channel,), __status_code)
...
def set_gain_out(channel: constants.Channel, mode: constants.GenGain) -> None:
    ...
    __status_code = rp.rp_GenSetGainOut(channel, mode)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenSetGainOut", (channel, mode), __status_code)
...
def get_gain_out(channel: constants.Channel) -> constants.GenGain:
    ...
    __status_code, __value = rp.rp_GenGetGainOut(channel)

    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetGainOut", (channel,), __status_code)
...
def set_ext_trigger_debouncer_us(value: float) -> None:
    ...
    if __status_code != StatusCode.OK:
        raise constants.RPPError(
            "rp_GenSetExtTriggerDebouncerUs", (value,), __status_code
        )
...
def get_ext_trigger_debouncer_us() -> float:
    ...
    if __status_code != StatusCode.OK:
        raise constants.RPPError("rp_GenGetExtTriggerDebouncerUs", (), __status_code)
